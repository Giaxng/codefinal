...
export function CollectionDock() {
    ...
    return (<div className="flex flex-col items-center gap-2">
      ...
      {selectedItem && selectedItem.collected && (<div style={{
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
            ...
            <button onClick={() => setSelectedId(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#94a3b8', fontSize: 14 }}>
              ✕
            </button>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4, fontSize: 10 }}>
            ...
            ].map(attr => (<div key={attr.label} style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 6, position: 'relative' }}>
          {collectedItems.map((item) => (<div key={item.id} onMouseEnter={() => setHoveredId(item.id)} onMouseLeave={() => setHoveredId(null)} onClick={() => item.collected && setSelectedId(selectedId === item.id ? null : item.id)} style={{
                width: 48,
                height: 48,
                borderRadius: 10,
              ...
              {item.collected ? (<span style={{ fontSize: 16, color: '#6366f1', fontWeight: 700, fontFamily: 'var(--font-serif)' }}>
                  {item.name[0]}
                </span>) : (<span style={{ fontSize: 18, color: '#cbd5e1' }}>?</span>)}

              
              {hoveredId === item.id && (<div style={{
                    position: 'absolute',
                    bottom: '100%',
                    left: '50%',
