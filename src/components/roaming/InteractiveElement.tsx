...
function CategoryGeometry({ category, collected }: {
    category: string;
    collected: boolean;
}) {
...
export function InteractiveElement({ item, position, rotation = [0, 0, 0], onCollect }: InteractiveElementProps) {
    ...
    return (<group ref={groupRef} position={position} rotation={rotation}>
      ...
      <group onClick={handleClick} onPointerOver={() => setIsHovered(true)} onPointerOut={() => setIsHovered(false)}>
        <CategoryGeometry category={item.category} collected={item.collected}/>
      </group>

      
      {!item.collected && (<Html position={[0, -0.5, 0]} center>
          <div style={{
      ...
      {isHovered && !showDetail && !item.collected && (<Html position={[0, 1.0, 0]} center>
          <div style={{
      ...
      {item.collected && (<Html position={[0, 0.6, 0]} center>
      ...
      {showDetail && (<Html position={[1.5, 0, 0]} center>
          <div style={{
                background: 'rgba(255,255,255,0.95)',
            ...
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
              ...
              <button onClick={() => setShowDetail(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#94a3b8', fontSize: 14 }}>
                ✕
              </button>
            </div>
            ...
            ].map(attr => (<div key={attr.label} style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 11 }}>
