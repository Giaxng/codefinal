...
export function RouteVisualizer({ startStation, endStation }: RouteVisualizerProps) {
    ...
    useFrame((state) => {
        if (particlesRef.current && curve) {
            ...
            particlesRef.current.children.forEach((child, i) => {
                ...
                const tt = (t * 0.5 + offset) % 1;
                const pos = curve.getPoint(tt);
                child.position.copy(pos);
                const scale = Math.sin(tt * Math.PI) * 1.5;
                child.scale.setScalar(scale);
            });
        }
    });
    if (!startStation || !endStation || !curve)
        return null;
    ...
    return (<group>
      ...
      <Line points={points} color={pathColor} lineWidth={4} transparent opacity={0.6} toneMapped={false}/>
      
      
      <Line points={points} color="#ffffff" lineWidth={1} transparent opacity={0.8} dashed dashSize={0.2} gapSize={0.1}/>

      
      <group ref={particlesRef}>
        {Array.from({ length: PARTICLE_COUNT }).map((_, i) => (<mesh key={i}>
      ...
      {midPoint && (<Html position={[midPoint.x, midPoint.y + 0.5, midPoint.z]} center>
          <div className="bg-black/60 backdrop-blur-md border border-white/20 rounded-lg p-3 min-w-[200px] pointer-events-none transform -translate-y-full shadow-[0_0_20px_rgba(34,211,238,0.2)]">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="font-mono font-bold text-lg" style={{ color: pathColor, textShadow: `0 0 10px ${pathColor}` }}>
                {mutationIndex}%
              </span>
            </div>
            ...
            <div className="h-px bg-white/10 mb-2"/>
             ...
             {boundaries.length > 0 && (<div className="flex flex-wrap gap-1">
                {boundaries.map((boundary, i) => (<span key={i} className="text-[10px] bg-amber-500/20 text-amber-300 px-1.5 py-0.5 rounded border border-amber-500/30">
                    {boundary}
                  </span>))}
              </div>)}
          </div>
