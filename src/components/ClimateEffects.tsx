...
export function ClimateEffects() {
    ...
    return (<group>
      {isRain ? (<RainSystem count={count}/>) : (<DustSystem count={count}/>)}
    </group>);
}
function RainSystem({ count }: {
    count: number;
}) {
    return (<Instances range={count}>
      <cylinderGeometry args={[0.01, 0.01, 0.5]}/>
      <meshBasicMaterial color="#a5f3fc" transparent opacity={0.4}/>
      {RAIN_PARTICLES.slice(0, count).map((data, i) => (<RainDrop key={i} data={data}/>))}
    </Instances>);
}
function RainDrop({ data }: {
    data: {
        position: [
            number,
            number,
    ...
    return (<group ref={ref} position={data.position}>
      <Instance />
    </group>);
}
function DustSystem({ count }: {
    count: number;
}) {
    return (<Instances range={count}>
      <dodecahedronGeometry args={[0.03, 0]}/>
      <meshBasicMaterial color="#fbbf24" transparent opacity={0.4}/>
      {DUST_PARTICLES.slice(0, count).map((data, i) => (<DustMolecule key={i} data={data}/>))}
    </Instances>);
}
interface DustData {
    position: [
