    ...
    return (<div style={{ width: '100%', height: '100%', position: 'relative' }}>
      ...
      <Canvas shadows style={{
        ...
        <Suspense fallback={<LoadingFallback />}>
          ...
          <OrbitControls enablePan enableZoom enableRotate enableDamping dampingFactor={0.08} minDistance={3} maxDistance={30} target={[0, 2, 0]} autoRotate autoRotateSpeed={0.5}/>
          <ambientLight intensity={0.7} color="#faf5ef"/>
          <directionalLight position={[8, 12, 8]} intensity={1.4} castShadow color="#fff7ed" shadow-mapSize={[2048, 2048]}/>
          <pointLight position={[-5, 5, -5]} intensity={0.4} color="#d4a574"/>
          
          <Environment blur={0.8}>
            <color attach="background" args={['#f5f0eb']}/>
          </Environment>
          <ContactShadows position={[0, -0.01, 0]} opacity={0.5} scale={30} blur={2.5} far={5} color="#8b7355"/>
