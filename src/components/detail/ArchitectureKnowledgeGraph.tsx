...
type DetailKey = keyof Pick<StationDetailData, 'classification' | 'distribution' | 'defense' | 'materials' | 'folklore' | 'heating' | 'moistureProof'>;
type GraphNode = {
    id: string;
    name: string;
    category: number;
...
export function ArchitectureKnowledgeGraph({ detail, stationName }: Props) {
        ...
        const nodes: GraphNode[] = [
            {
                ...
                itemStyle: {
                    ...
                    borderWidth: 3,
                    shadowBlur: 18,
                    shadowColor: 'rgba(163,129,84,0.20)',
                },
                label: {
                    color: '#23180f',
                    fontSize: 14,
        ...
        CATEGORY_META.forEach((meta, index) => {
            ...
            nodes.push({
                ...
                label: {
                    ...
                    width: 64,
                    overflow: 'break',
                    textBorderColor: 'rgba(252, 248, 242, 0.92)',
                    textBorderWidth: 3,
                },
            });
            links.push({
            ...
            extractFragments(content).forEach((fragment, fragmentIndex) => {
                ...
                nodes.push({
                    ...
                    category: index + 1,
                    symbolSize: 18 + Math.min(fragment.length * 0.35, 8),
                    draggable: true,
                    value: fragment,
                    itemStyle: {
                        color: '#fff9f0',
            ...
            series: [
                {
                    ...
                    label: {
                        ...
                        formatter: '{b}',
                        color: '#2f241b',
                        textBorderColor: 'rgba(252, 248, 242, 0.95)',
                        textBorderWidth: 3,
                    },
                    emphasis: {
                        focus: 'adjacency',
    ...
    return (<div style={{
            ...
            borderRadius: 18,
            overflow: 'hidden',
            background: 'radial-gradient(circle at 20% 20%, rgba(196,168,130,0.10) 0%, transparent 24%), linear-gradient(180deg, #fcf8f2 0%, #f5ede2 100%)',
        }}>
      <div aria-hidden="true" style={{
            position: 'absolute',
            inset: 0,
            backgroundImage: 'linear-gradient(rgba(139,115,85,0.04) 1px, transparent 1px), linear-gradient(90deg, rgba(139,115,85,0.04) 1px, transparent 1px)',
      ...
      <div style={{
        ...
        {[
            ...
            { label: '重置', onClick: () => setZoom(initialZoom) },
            { label: '＋', onClick: () => setZoom((current) => Math.min(1.8, Number((current + 0.15).toFixed(2)))) },
        ].map((action) => (<button key={action.label} type="button" onClick={action.onClick} style={{
      ...
      <ReactECharts ref={chartRef} option={option} opts={{ renderer: 'canvas' }} style={{
            width: '100%',
            height: '100%',
            cursor: isDragging ? 'grabbing' : 'grab',
        }} notMerge={true} lazyUpdate={true} onChartReady={handleChartReady}/>
    </div>);
}
