...
export function DecorationPanel({ decorations, stationName }: Props) {
    ...
    return (<div style={{ marginBottom: 48 }}>
      ...
      {Object.entries(groupedByForm).map(([form, items]) => (<div key={form} style={{ marginBottom: 20 }}>
          <div style={{
                display: 'flex',
                alignItems: 'center',
                gap: 8,
          ...
          <div style={{
            ...
            {items.map((item, idx) => (<div key={idx} style={{
                ...
                {item.feature && (<div style={{ fontSize: 11, color: '#9ca3af', marginTop: 4, fontStyle: 'italic' }}>
                    {item.feature}
                  </div>)}
                {item.folklore && (<div style={{ fontSize: 11, color: '#92400e', marginTop: 4 }}>
