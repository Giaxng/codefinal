...
function TransitionContent() {
    return (<>
      ...
      <OrbitControls enablePan={false} enableZoom enableRotate minDistance={4} maxDistance={15} target={[0, 1, 0]} autoRotate autoRotateSpeed={0.5}/>
...
export function TransitionScene({ onComplete, onBack }: TransitionSceneProps) {
    ...
    const morphDesc = (() => {
        if (!startStation || !endStation)
            return { title: '过渡型', desc: '建筑形态正在演变中...' };
        const t = transitionProgress;
        if (t < 0.3)
            return { title: startStation.buildingGene, desc: `${startStation.name}形态：${startStation.description}` };
        if (t < 0.7)
            return { title: '过渡形态', desc: `正在从「${startStation.buildingGene}」向「${endStation.buildingGene}」演变` };
    ...
    if (startStation && endStation) {
        ...
        for (const line of [800, 400]) {
            if ((sr > line && er < line) || (sr < line && er > line)) {
                ...
                if (t > 0 && t < 1)
                    keyNodes.push({ pos: t, label: `${line}mm线` });
            }
        }
    }
    return (<>
      ...
      <div style={{ position: 'fixed', inset: 0, zIndex: 20, pointerEvents: 'none' }}>
        ...
        {transitionProgress >= 0.9 && (<button onClick={handleComplete} style={{
        ...
        <div style={{
          ...
          <div style={{ ...glass, padding: infoExpanded ? '14px 18px' : '10px 16px', cursor: 'pointer', transition: 'all 0.3s' }} onClick={() => setInfoExpanded(!infoExpanded)}>
            ...
            {infoExpanded && (<div style={{ fontSize: 12, color: '#64748b', lineHeight: 1.6, marginTop: 8, borderTop: '1px solid rgba(0,0,0,0.06)', paddingTop: 8 }}>
                {morphDesc.desc}
              </div>)}
          </div>
        </div>
        ...
        <div style={{
          ...
          <div style={{ ...glass, padding: '14px 20px' }}>
            ...
            <div className="relative">
              <input type="range" min="0" max="1" step="0.001" value={transitionProgress} onChange={(e) => setTransitionProgress(Number(e.target.value))} className="w-full h-2 bg-slate-200 rounded-lg appearance-none cursor-pointer 
                           [&::-webkit-slider-thumb]:appearance-none 
                           [&::-webkit-slider-thumb]:w-5 [&::-webkit-slider-thumb]:h-5 
                           [&::-webkit-slider-thumb]:rounded-full [&::-webkit-slider-thumb]:bg-blue-500
                           [&::-webkit-slider-thumb]:shadow-[0_0_12px_rgba(37,99,235,0.4)]
                           [&::-webkit-slider-thumb]:border-2 [&::-webkit-slider-thumb]:border-white"/>
              {keyNodes.map((node, i) => (<div key={i} className="absolute top-1/2 -translate-y-1/2 pointer-events-none" style={{ left: `${node.pos * 100}%` }}>
