...
export function NarratorPanel() {
    ...
    const narrations = useMemo(() => {
        return generateNarrations(startStation?.name || '起点', endStation?.name || '终点', startStation?.buildingGene || '传统', endStation?.buildingGene || '传统', startStation?.description || '传统民居', endStation?.description || '传统民居');
    }, [startStation, endStation]);
    const currentNarration = useMemo(() => {
        return narrations.find((n) => transitionProgress >= n.progressStart && transitionProgress < n.progressEnd);
    ...
    const formattedText = useMemo(() => {
        if (!currentNarration || !displayedText)
    ...
    return (<div className="absolute bottom-24 left-1/2 -translate-x-1/2 z-10 w-[600px] max-w-[90vw]">
      <div style={{
        ...
        <div style={{ color: '#334155', lineHeight: 1.7, minHeight: 48, fontSize: 14 }}>
          {formattedText}
          {isTyping && (<span style={{ color: '#6366f1', animation: 'pulse 1s infinite' }}>|</span>)}
        </div>
